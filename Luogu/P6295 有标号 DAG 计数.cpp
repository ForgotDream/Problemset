...
constexpr i64 mod = 998244353, g0 = 3;
...
void ntt(iter beg, iter end, int opt) {
  ...
  for (int h = 2; h <= len; h <<= 1) {
    i64 delta = fast_pow(g0, (mod - 1) / h, mod);
...
void solve() {
  ...
  ifc[n] = fast_pow(fac[n], mod - 2, mod);
  ...
  for (int i = 1; i <= n; i++) {
    i64 d = 1ll * i * (i - 1) / 2 % (mod - 1);
    g[i] = (i & 1 ? mod - 1 : 1) * inv(fast_pow(2, d, mod)) % mod * ifc[i] % mod;
  }
  g[0] = (g[0] + 1) % mod;

  for (int i = 0; i <= n; i++) std::cerr << g[i] << " \n"[i == n];

  g = inv(g.begin(), g.end());
  for (int i = 0; i <= n; i++) {
    i64 d = 1ll * i * (i - 1) / 2 % (mod - 1);
    g[i] = g[i] * fast_pow(2, d, mod) % mod;
  }
  g = ln(g.begin(), g.end());

  for (int i = 1; i <= n; i++) std::cout << g[i] * fac[i] % mod << "\n";
}
