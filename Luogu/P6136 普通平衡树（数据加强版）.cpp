...
const int N = 1e6 + 1e5 + 50;
int n, m, opt, optnum, last, ans;
...
int main() {
	srand(time(0));
	read(n, m);
	for (int i = 1, tmp; i <= n; i++) read(tmp), T.insert(T.root, tmp);
	while (m--) {
		read(opt, optnum);
		optnum ^= last;
		if (opt == 1) T.insert(T.root, optnum);
		else if (opt == 2) T.del(T.root, optnum);
		else if (opt == 3) last = T.getRankByNum(T.root, optnum) + 1, ans ^= last;
		else if (opt == 4) last = T.getNumByRank(T.root, optnum), ans ^= last;
		else if (opt == 5) last = T.getPrecursor(T.root, optnum), ans ^= last;
		else last = T.getSuccessor(T.root, optnum), ans ^= last;
	}
	print(ans, '\n');
	return 0;
}