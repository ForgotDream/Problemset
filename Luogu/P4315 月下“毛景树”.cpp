...
int main() {
	...
	while (opt[0] != 'S') {
		// cerr << 1 << endl;
		if (opt[1] == 'h') 
			read(x, z), y = edge[x].v, x = edge[x].u, 
			dep[x] > dep[y] ? assign(no[x], no[x], 1, n, 1, z) : assign(no[y], no[y], 1, n, 1, z);
		else if (opt[1] == 'o') read(x, y, z), assignP2P(x, y, z);
		else if (opt[1] == 'd') read(x, y, z), addP2P(x, y, z);
