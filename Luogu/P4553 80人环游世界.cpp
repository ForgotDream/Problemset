...
signed main() {
  ...
  for (int i = 1; i <= n; i++) {
    int v;
    std::cin >> v;
    solver.addEdge(S, i, v, 0);
    solver.addEdge(i + n, T, v, 0);
  }
  for (int i = 1; i <= n; i++) {
    for (int j = i + 1; j <= n; j++) {
      int cost;
      std::cin >> cost;
      if (cost == -1) continue;
      solver.addEdge(i, j + n, 1e9, cost);
    }
  }
  int tmp = 100050 - 2;
  solver.addEdge(S, tmp, m, 0);
  for (int i = 1; i <= n; i++) solver.addEdge(tmp, i + n, 1e9, 0);
