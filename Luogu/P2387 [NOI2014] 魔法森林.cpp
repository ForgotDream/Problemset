/*
 * @file    P2387 [NOI2014] 魔法森林.cpp
 * @author  ForgotDream
 * @brief   LCT
 * @date    2023-12-28
 */
#include <bits/stdc++.h>

using i64 = long long;

constexpr int N = 2e5 + 50;
int n, m;
struct Edge {
  int u, v, w1, w2;
  bool operator<(const Edge &rhs) const { return w1 < rhs.w1; }
} edges[N];

struct LCT {
  int ch[N][2], siz[N], tag[N], fa[N], val[N], max[N], idx[N];
  int d(int u) { return ch[fa[u]][1] == u; }
  bool isRoot(int u) { return ch[fa[u]][0] != u && ch[fa[u]][1] != u; }
  void pushup(int u) { 
    siz[u] = siz[ch[u][0]] + siz[ch[u][1]] + 1; 
    max[u] = val[u], idx[u] = u;
    if (max[ch[u][0]] > max[u]) max[u] = max[ch[u][0]], idx[u] = idx[ch[u][0]];
    if (max[ch[u][1]] > max[u]) max[u] = max[ch[u][1]], idx[u] = idx[ch[u][1]];
  }
  void pushdown(int u) {
    if (!tag[u]) return;
    tagging(ch[u][0]), tagging(ch[u][1]);
    tag[u] = false;
  }
  void tagging(int u) {
    if (!u) return;
    std::swap(ch[u][0], ch[u][1]), tag[u] ^= true;
  }
  void rotate(int u) {
    int f = fa[u], g = fa[f], k = d(u);
    if (!isRoot(f)) ch[g][d(f)] = u;
    ch[f][k] = ch[u][!k], fa[ch[u][!k]] && (fa[ch[u][!k]] = f);
    ch[u][!k] = f, fa[f] = u, fa[u] = g;
    pushup(f), pushup(u);
  }
  void update(int u) {
    if (!isRoot(u)) update(fa[u]);
    pushdown(u);
  }
  void splay(int u) {
    update(u);
    for (int f = fa[u]; !isRoot(u); rotate(u), f = fa[u]) {
      if (!isRoot(f)) rotate(d(f) == d(u) ? f : u);
    }
  }
  void access(int u) {
    for (int c = 0; u; c = u, u = fa[u]) splay(u), ch[u][1] = c, pushup(u);
  }
  void makeRoot(int u) { access(u), splay(u), tagging(u); }
  int findRoot(int u) {
    access(u), splay(u);
    while (ch[u][0]) pushdown(u), u = ch[u][0];
    return u;
  }
  void link(int u, int v) { makeRoot(u), fa[u] = v; }
  void split(int u, int v) { makeRoot(u), access(v), splay(v); }
  void cut(int u, int v) {
    split(u, v);
    if (ch[v][0] != u || ch[u][1]) return;
    fa[u] = ch[v][0] = 0;
    pushup(v);
  }
} lct;

void solve() {
  std::cin >> n >> m;
  for (int i = 1; i <= m; i++) {
    auto &[u, v, w1, w2] = edges[i];
    std::cin >> u >> v >> w1 >> w2;
  }

  std::sort(edges + 1, edges + m + 1);
  for (int i = 1; i <= m; i++) {
    lct.val[i + n] = lct.max[i + n] = edges[i].w2;
    lct.idx[i + n] = i + n;
  }

  int ans = 2e9;
  for (int i = 1; i <= m; i++) {
    auto [u, v, w1, w2] = edges[i];
    // std::cerr << u << " " << v << " " << w1 << " " << w2 << "\n";
    if (u == v) continue;
    if (lct.findRoot(u) != lct.findRoot(v)) {
      lct.link(u, i + n), lct.link(i + n, v);
    } else {
      lct.split(u, v);
      int p = lct.idx[v], mx = lct.max[v];
      // std::cerr << p << " " << mx << " " << w2 << "\n";
      if (mx <= w2) continue;
      lct.splay(p), lct.fa[lct.ch[p][0]] = lct.fa[lct.ch[p][1]] = 0;
      lct.link(u, i + n), lct.link(i + n, v);
    }
    if (lct.findRoot(1) == lct.findRoot(n)) {
      lct.split(1, n);
      ans = std::min(ans, w1 + lct.max[n]);
    }
  }

  std::cout << (ans == 2e9 ? -1 : ans) << "\n";
}

int main() {
  std::cin.tie(nullptr)->sync_with_stdio(false);

  int t = 1;
  // std::cin >> t;
  while (t--) solve();

  return 0;
}
