...
struct SegTree {
  ...
  void build(int s, int t, int u) {
    if (s == t) return mn[u] = a[s], ss[u] = 6 * a[s] * a[s], void();
    int mid = (s + t) >> 1;
    build(s, mid, lc(u)), build(mid + 1, t, rc(u));
    pushup(u);
  }
  void modify(int tar, int s, int t, int u, u64 val) {
    if (s == t) return mn[u] = val, ss[u] = 6 * val * val, void();
    int mid = (s + t) >> 1;
    if (mid >= tar) modify(tar, s, mid, lc(u), val);
    else modify(tar, mid + 1, t, rc(u), val);
    pushup(u);
  }
  pil query(int l, int r, int s, int t, int u) {
    if (l <= s && t <= r) return {mn[u], ss[u]};
    int mid = (s + t) >> 1;
    if (mid >= r) {
      ...
      auto [lmn, lss] = query(l, r, s, mid, lc(u));
      auto [rmn, rss] = query(l, r, mid + 1, t, rc(u));
      return {std::min(lmn, rmn), lss + rss};
    }
  }
} seg;
u64 squareSum(u64 u) {
...
void solve() {
  ...
  seg.build(1, n, 1);
  int cnt = 0;
  for (int opt, x, y, k; m; m--) {
    fin >> opt >> x >> y, x ^= cnt, y ^= cnt;
    if (opt == 1) {
      seg.modify(x, 1, n, 1, y);
    } else {
      fin >> k, k ^= cnt;
      auto [mn, ss] = seg.query(x, y, 1, n, 1);
      u64 len = y - x + 1;
      u64 cur = 6ul * len * mn * mn;
      cur = cur + 6ul * mn * k * len * (len - 1);
      cur = cur + k * k * len * (len - 1) * (2 * len - 1);
      // std::cerr << cur << " " << ss << "\n";
      if (cur == ss) fout << "Yes\n", cnt++;
      else fout << "No\n";
    }
