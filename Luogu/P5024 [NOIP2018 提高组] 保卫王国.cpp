/*
 * @file    P5024 [NOIP2018 提高组] 保卫王国.cpp
 * @author  ForgotDream
...
inline void modify(int u, i64 w) {
  mat[u][1][0] += w - p[u];
  p[u] = w;
...
void solve() {
  ...
  for (int i = 1, u, a, v, b; i <= m; i++) {
    std::cin >> u >> a >> v >> b;
    i64 pu = p[u], pv = p[v], d = sum - pu - pv;
    modify(u, a ? -inf : inf), modify(v, b ? -inf : inf);
    d += (a ? pu : inf) + (b ? pv : inf);
    auto ansm = seg.query(in(1), out(1), 1, n, 1);
    i64 ans = d - std::max(ansm[0][0], ansm[1][0]);
    if (ans > inf) std::cout << -1 << "\n";
    else std::cout << ans << "\n";
    modify(u, pu), modify(v, pv);
  }
}
