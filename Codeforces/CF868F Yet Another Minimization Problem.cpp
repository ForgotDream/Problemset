/**
 * @file    CF868F Yet Another Minimization Problem.cpp
 * @author  ForgotDream
 * @brief   DP
 * @date    2023-07-26
 */
...
#define c1 cnt[a[s]]
#define c2 cnt[a[t]]
  while (s > l) s--, cur -= c1 * (c1 - 1) / 2, c1++, cur += c1 * (c1 - 1) / 2;
  while (s < l) cur -= c1 * (c1 - 1) / 2, c1--, cur += c1 * (c1 - 1) / 2, s++;
  while (t < r) t++, cur -= c2 * (c2 - 1) / 2, c2++, cur += c2 * (c2 - 1) / 2;
  while (r < t) cur -= c2 * (c2 - 1) / 2, c2--, cur += c2 * (c2 - 1) / 2, t--;
  return cur;
#undef c1
#undef c2
}
