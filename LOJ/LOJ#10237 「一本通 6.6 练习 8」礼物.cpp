/**
 * @file    LOJ#10237 「一本通 6.6 练习 8」礼物.cpp
 * @author  ForgotDream
 * @brief   Math
 * @date    2023-08-11
 */
...
signed main() {
  ...
  std::cin.tie(nullptr);
  std::cin >> p >> n >> m;
  for (int i = 1; i <= m; i++) std::cin >> w[i], sum += w[i];
  if (sum > n) return std::cout << "Impossible\n", 0;
  i64 ans = 1;
  for (int i = 1; i <= m; i++) (ans *= exLucas(n, w[i], p)) %= p, n -= w[i];
  std::cout << ans << "\n";
  return 0;
}