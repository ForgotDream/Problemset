...
void init(int _n) {
  ...
  for (int i = 1; i <= _n; i++) {
    s_phi[i] = (s_phi[i] + phi[i] * i % p * i % p) % p;
  }
}
...
inline i64 calc_s2(i64 u) {
  return u * (u + 1) % p * (2 * u + 1) % p * inv6 % p;
}

inline i64 calc_s3(i64 u) {
  i64 s1 = u * (u + 1) % p * inv2 % p;
  return s1 * s1 % p;
}
...
i64 calc(i64 u) {
  if (u <= n) return s_phi[u];
  if (mp_phi.count(u)) return mp_phi[u];

  i64 res = u * (u + 1) % p * inv2 % p;
  for (i64 i = 2, j; i <= u; i = j + 1) {
    j = u / (u / i);
    res = (res - calc(u / i) * ((calc_s2(j) - calc_s2(i - 1) + p) % p) % p) % p;
    res = (res + p) % p;
  }
